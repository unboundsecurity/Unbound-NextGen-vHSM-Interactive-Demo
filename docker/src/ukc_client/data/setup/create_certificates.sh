...
if ! ucl list -n ca -t ECC | grep ECC; then
  echo "# Create ca key"
  ucl generate -t ECC -n ca --user user 2>/dev/null

  echo "# Export the CA key"
  ucl export -n ca -o /certs/ca.key --obfuscate 2>/dev/null
...
if ! ucl list -n ca | grep Certificate; then
  ...
  ucl import --input /certs/ca.crt  --user user --name ca 2>/dev/null
fi


if ! ucl list -n codeSigning -t RSA | grep RSA ; then
  echo "# Create codeSigning key"
  ucl generate -t RSA -n codeSigning --user user 2>/dev/null
  
  echo "# Export the codeSigning  key"
  ucl export -n codeSigning -o /certs/code_signing.key --obfuscate 2>/dev/null

  echo "# Create certificate request."
  #ucl csr -o /certs/code_signing.csr --subject "CN=code Signing" -n codeSigning --user user
  openssl req -new -sha256 -key /certs/code_signing.key -out /certs/code_signing.csr -subj '/CN=code Signing' 2>/dev/null
