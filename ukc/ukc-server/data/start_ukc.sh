...
copy_casp_key() {
  if [ -n $USE_CASP ] && [[ $USE_CASP =~ ^(true|TRUE|yes|YES) ]] && [ -e /unbound/data/casp_backup.pem ]
  then
    echo "Copying CASP backup.pem"
    cp /unbound/data/casp_backup.pem /etc/ekm
  fi
}
...
case "$1" in
  ep)
    ep $2 $3
    if [ -e "/frontail/bin/frontail" ]; then
